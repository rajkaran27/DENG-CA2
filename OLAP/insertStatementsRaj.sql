...
SELECT p.product_id, c.categoryKey, b.brandKey, p.product_name, p.model_year, p.list_price, 0 AS stock_quantity, GETDATE() AS stock_date
...
// more to fix
// every product must have stock from all stores, if not should be 0
...
FROM production.products p
JOIN 
    BikeSalesDWGroup3..CategoryDIM c ON p.category_id = c.category_id
JOIN 
    BikeSalesDWGroup3..BrandDIM b ON p.brand_id = b.brand_id
LEFT JOIN 
    (SELECT 
         product_id, 
